...
describe('DELETE /api/v1/booking/:id', () => {
      ...
      afterAll(async () => {
        if (demoid) {
          await supertest(app)
            .delete(`/api/v1/booking/${createdBookingId}`);
        }
      });
...
describe('booking-with error cases',()=>{
  ...
  describe('DELETE/ booking',()=>{
    it('should return 404 if there is an error in deleting the booking', async () => {
      const { body, statusCode } = await supertest(app)
        .delete(`/api/v1/booking/662f73231a0399a6e66e08bc`)
      expect(statusCode).toBe(404);
      expect(body.success).toBe(false);
    });
    ...
    it('should return 500 if there is an error in deleting the booking', async () => {
      const { body, statusCode } = await supertest(app)
        .delete(`/api/v1/booking/662f73231a0399a6e66`)
...
describe("POST/ booking",()=>{
    it('should enter empty body and return 404',async()=>{
      const {statusCode} = await supertest(app).post('/api/v1/booking/').send({})
      expect(statusCode).toBe(404)
    })
    it('should create a booking and return 500',async()=>{
      const booking_payload={
          pickupLocation:"Panchamrut Bunglows II, Sola, Ahmedabad, Gujarat 380059, India",
          dropoffLocation:"Sola, Ahmedabad, Gujarat 380059, India",
          vehicleClass:"Auto",
          fare:140,
           status: "pendingwde"
      }
      const {statusCode} = await supertest(app).post('/api/v1/booking/').send(booking_payload);    
        expect(statusCode).toBe(500)
    })
...
describe('PUT/ booking',()=>{
    it('should return 500 if there is an error in updateing the booking', async () => {
       const { body, statusCode } = await supertest(app)
         .put(`/api/v1/booking/662f73231a0399a`)
       expect(statusCode).toBe(500);
       expect(body.success).toBe(false);
     });
